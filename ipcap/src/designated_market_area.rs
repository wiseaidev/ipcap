...
/// Lazy static HashMap containing DMA (Designated Market Area) codes and their corresponding locations.
///
/// The DMA map provides a mapping between DMA codes (i32) and their associated location names.
/// It uses the `once_cell` crate's `Lazy` to ensure lazy initialization and thread safety.
pub static DMAS: Lazy<HashMap<i32, &'static str>> = Lazy::new(|| {
    [
...
mod tests {
    ...
    #[test]
    fn test_dma_lookup() {
        // Test DMA code lookup for Portland-Auburn, ME
        let location = DMAS.get(&500);
        assert_eq!(location, Some(&"Portland-Auburn, ME"));

        // Test DMA code lookup for New York, NY
        let location = DMAS.get(&501);
        assert_eq!(location, Some(&"New York, NY"));

        // Test DMA code lookup for an invalid code
        let location = DMAS.get(&999);
